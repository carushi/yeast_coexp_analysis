...
print.KEGG.enrichment <- function(gene.data, all.genes, outfile, kegg.code) {
    ...
    for (change in c("up", "down", "change")) {
        ...
        print(c(change, length(part.genes.ez)))
...
dir = "./"
id_dir = "../../../ann/"
for (fname in list.files(dir, pattern="genotype*", full.names=TRUE)) {
    ...
    print(base_prefix)
    plot.KEGG.enrichment(data[,2], data[,6], 0.05, rownames(data), paste0("kegg_", base_prefix), file.path(id_dir, "kegg_id_list.txt"))
    plot.GO.enrichment(data[,2], data[,6], 0.05, rownames(data), paste0("go_", base_prefix))
}
for (fname in list.files(dir, pattern="time*", full.names=TRUE)) {
    data <- read.table(fname, header=T, sep="\t")
    print(head(data))
    data <- subset(data, !is.na(rownames(data)))
    base_prefix = unlist(strsplit(basename(fname), ".", fixed=TRUE))[1]
